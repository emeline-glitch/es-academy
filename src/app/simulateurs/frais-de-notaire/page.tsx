...
// Départements à taux réduit (3 départements + Mayotte)
const DEPARTEMENTS_REDUITS = ["971", "972", "973", "974", "976"];
    ...
    <div className="min-h-screen bg-es-cream">
      ...
      <section className="py-12">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <p className="text-es-text-muted mb-8">Estimez les frais de notaire pour votre achat immobilier</p>

          <div className="grid lg:grid-cols-2 gap-8">
            <div className="space-y-5">
              <Card>
                <h3 className="font-medium text-gray-900 mb-4">Votre achat</h3>
                <div className="space-y-4">
                  <div>
                    <label className="text-sm text-gray-600 mb-1 block">Prix d&apos;achat : <strong>{prix.toLocaleString("fr-FR")}€</strong></label>
                    <input type="range" min={50000} max={1000000} step={5000} value={prix} onChange={(e) => setPrix(Number(e.target.value))} className="w-full accent-es-green" />
                  </div>
                  <div>
                    <label className="text-sm text-gray-600 mb-2 block">Type de bien</label>
                    <div className="flex gap-2">
                      <button onClick={() => setType("ancien")} className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all cursor-pointer ${type === "ancien" ? "bg-es-green text-white" : "bg-gray-100 text-gray-600"}`}>
                        Ancien
                      </button>
                      <button onClick={() => setType("neuf")} className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all cursor-pointer ${type === "neuf" ? "bg-es-green text-white" : "bg-gray-100 text-gray-600"}`}>
                        Neuf (VEFA)
                      </button>
                    </div>
                  </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-amber-50 rounded-xl p-4 border border-amber-200 text-sm text-amber-800">
                💡 <strong>Astuce :</strong> Vous pouvez négocier les émoluments du notaire sur la partie au-dessus de 100 000€ (jusqu&apos;à -20%). Demandez une remise !
              </div>

              <div className="bg-es-green/5 rounded-xl p-6 border border-es-green/10 text-center">
                <p className="text-sm text-es-text-muted mb-4">Découvrez comment réduire vos frais de notaire dans notre formation.</p>
