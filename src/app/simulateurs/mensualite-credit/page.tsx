...
export default function MensualiteCredit() {
  ...
  const mensualite = mensualiteTotal;
    ...
    <div className="min-h-screen bg-es-cream">
      ...
      <section className="py-12">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <p className="text-es-text-muted mb-8">Calcule ta mensualité en fonction du montant, taux et durée</p>

          <div className="grid lg:grid-cols-2 gap-8">
            <Card>
              <h3 className="font-medium text-gray-900 mb-4">Ton crédit</h3>
              <div className="space-y-4">
                <div>
                  <label className="text-sm text-gray-600 mb-1 block">Montant emprunté : <strong>{montant.toLocaleString("fr-FR")}€</strong></label>
                  <input type="range" min={30000} max={800000} step={5000} value={montant} onChange={(e) => { setMontant(Number(e.target.value)); markCalculated(); }} className="w-full accent-es-green" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 mb-1 block">Durée : <strong>{duree} ans</strong></label>
                  <input type="range" min={5} max={25} step={1} value={duree} onChange={(e) => { setDuree(Number(e.target.value)); markCalculated(); }} className="w-full accent-es-green" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 mb-1 block">Taux d&apos;intérêt : <strong>{taux}%</strong></label>
                  <input type="range" min={0.5} max={7} step={0.1} value={taux} onChange={(e) => { setTaux(Number(e.target.value)); markCalculated(); }} className="w-full accent-es-green" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 mb-1 block">Assurance : <strong>{assurance}%</strong></label>
                  <input type="range" min={0} max={0.6} step={0.02} value={assurance} onChange={(e) => { setAssurance(Number(e.target.value)); markCalculated(); }} className="w-full accent-es-green" />
                </div>
              </div>
            </Card>
            ...
            <div className="space-y-5">
              <Card className="bg-es-green text-white border-0">
                <h3 className="text-white/60 text-sm mb-4">Ta mensualité</h3>
          ...
          <SimulatorCapture
            simulatorType="mensualite-credit"
            hasCalculated={hasCalculated}
            formInputs={{ montant, duree, taux, assurance }}
            formOutputs={{ mensualiteHorsAss, mensualiteAssurance, mensualiteTotal, coutTotal, coutInterets }}
            nextStepTitle={`Ta mensualité est de ${Math.round(mensualite).toLocaleString("fr-FR")}€ ?`}
            nextStepBody="On analyse ensemble si ton bien s'autofinance vraiment, en tenant compte des charges, de la fiscalité et de la vacance locative."
          />
        </div>
      </section>
      <Footer />
