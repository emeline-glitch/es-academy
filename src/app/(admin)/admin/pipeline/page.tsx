  ...
  return (
    <div>
      ...
      {/* Tabs pour switcher entre les 3 pipelines.
          Pipeline vide -> onglet visible mais sobre, jamais cache : Family
          et Sur-mesure doivent rester accessibles meme quand ils sont vides
          au demarrage. */}
        ...
        {(Object.keys(PIPELINES) as PipelineType[]).map((t) => {
          ...
          return (
            <button
              ...
              title={isEmpty ? `Pipeline ${cfg.label} vide pour l'instant` : undefined}
            >
              <span className="mr-1.5">{cfg.icon}</span>
              {cfg.label}
              <span className={`ml-2 text-[10px] px-1.5 py-0.5 rounded-full ${active ? "bg-es-green/10 text-es-green" : isEmpty ? "bg-gray-100 text-gray-500" : "bg-gray-200 text-gray-700"}`}>
                {count}
              </span>
            </button>
          );
